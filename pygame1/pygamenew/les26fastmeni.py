...
class Game:
    def __init__(self):
        ...
        # Загрузка графики
        self.background = pygame.image.load(path.join(img_dir, "фон2.jpg")).convert()
        ...
        self.player_img = pygame.image.load(path.join(img_dir, "игроксверху.png")).convert()
        self.player_img = pygame.transform.scale(player_img, (50, 60))

        self.mob_img = pygame.image.load(path.join(img_dir, "ghost.png")).convert()
        self.mob_img = pygame.transform.scale(mob_img, (50, 60))
        self.bullet_img = pygame.image.load(path.join(img_dir, "bullet.png")).convert()
    ...
    # Пули
        ...
        self.shopMenuName = "Shop menu"

        # Счет
        self.score = 0  # 20
        self.gold = 0  # 20 #золото
    ...
    # кнопки
        button_x = WIDTH - BUTTON_WIDTH - self.PADDING  # 22 3 переменная для координаты x кнопки

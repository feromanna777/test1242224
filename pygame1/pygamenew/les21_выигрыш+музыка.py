...
win_sound=pygame.mixer.Sound("Victory Screen Appear 01.wav")
...
start_time = time.time()#время начало игры
print(start_time)
# Цикл игры
while True:
    print(start_time)
...
#выигрыш
    ...
    if time.time() - start_time > 5.0: #22смотрим время дя выигрыша
        pygame.mixer.music.fadeout(3)#fadeout(3): Это метод, вызванный на объекте pygame.mixer.music,
        # и он используется для постепенного уменьшения громкости звукового трека до полного его выключения за указанное количество секунд.

#3 в данном случае обозначает количество секунд, за которое будет происходить затухание музыки.
        # В данном примере это 3 секунды. Вы можете изменить это значение в зависимости от ваших предпочтений.
        win_sound.play()
        while True:
            clock.tick(FPS)
        ...
        #меняем задний фон
            background = pygame.image.load(path.join(img_dir, "победа.png")).convert()  # изменение фона при игра окончена
            background = pygame.transform.scale(background, (WIDTH, HEIGHT))
            screen.blit(background, background_rect)


            draw_text(screen, "You win! Your score: " + str(score), 35, 240, 300, RED)
            pygame.display.flip()

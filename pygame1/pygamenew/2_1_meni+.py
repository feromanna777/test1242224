                ...
                for hit in hits:  # 21 наносим урон персонажу
                    ...
                    if player.health <= 0:  # 21
                        while True:
                            ...
                            screen.blit(background, self.background_rect)  # 19
                            all_sprites.draw(screen)

                            draw_text(screen, "GAME OVER! Your score: " + str(self.score), 35, 240, 300, RED)  # 20
                            # draw_hp(screen, 5, 5, player.health)
                            pygame.display.flip()

            # Рендеринг (отрисовка)
            screen.fill(BLACK)
            screen.blit(self.background, self.background_rect)  # 19

            all_sprites.draw(screen)
            draw_text(screen, "счет " + str(self.score), 20, 50, 20, WHITE)  # 19
            draw_text(screen, "золото " + str(self.gold), 20, 50, 40, YELLOW)  # золото
            draw_hp(screen, 5, 5, player.health)

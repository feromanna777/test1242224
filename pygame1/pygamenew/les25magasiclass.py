...
class Item:  # 23классс педмета
    def __init__(self, name, price, file):
        ...
        self.price = price
        self.file = file
        self.image = load_image(file, PLAYER_WIDTH // 1.7,
                                PLAYER_HEIGHT // 1.7)  # 23 magas картинка для меню чуть меньшего размера чем полная картинка
...
class ShopMenu:  # ! 23классс менюодежды
    def __init__(self):  # 23 game ссылка на сам класс игры чтобы загружать деньги потом
        ...
        self.items = [Item("cиняя футболка", 30, "img/items/синяя_футболка.png"),  # 23список всех предметов в магазине
                      Item("пистолет", 30, "img/items/синяя_футболка.png")]  # 23
    ...
    def to_next(self):  # 23 наметка позже напишем
        if self.current_item != len(self.items) - 1:  # 23 проверяем что кнопка не последняя
...
def shop_menu_on():# 23.2 функция влючения меню
    ...
    mode = shopMenuName  # 23.2 переключает меню на включено

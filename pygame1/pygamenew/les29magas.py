...
class ShopMenu:  # ! 23классс менюодежды
    def __init__(self):  # 23 game ссылка на сам класс игры чтобы загружать деньги потом
        ...
        self.items = [Item("cиняя футболка", 30, "img/items/синяя_футболка.png"),  # 23список всех предметов в магазине
                      ...
                      Item("паук", 700, "img/items/паук-removebg-preview (1).png")]  # 23
...
#not self.items[self.current_item].is_using: Эта часть кода инвертирует текущее значение атрибута is_using для выбранного предмета. Если is_using было True, то после выполнения этой строки оно станет False, и наоборот.
    ...
    def draw(self, screen):  # 23 добавляем функцию для отрисовки с параметром экран
        ...
        if self.items[self.current_item].is_bought:  # 23 проверяем куплен ли предмет если да
            # screen.blit(self.bottom_label_on, (0, 10))  # то отрисовывем боттом лэйбл он
    ...
    def update(self):  # 3метод обновления кнопки
        ...
        if self.rect.collidepoint(mouse_pos):  # 3 если позиция кнопки пересекается с мышкой
            if self.is_pressed:  # 3 если кнопка нажата
                ...
                self.image = self.idle_image  # 3 иначе картинка будет не нажатой кнопкой
...
while True:
    ...
    for e in pygame.event.get():
        ...
        # if mode=="Main":
        #     #если игра не идет при клике переход в меню
        #     if e.type == pygame.KEYDOWN:
        #         ==1:
        #         mode="menu"
        ...
        if hits:
            for hit in hits:  # 21 наносим урон персонажу
                ...
                if player.health <= 0:  # 21
                    ...
                    with open("24_2золото.txt", "w", encoding="utf-8") as f:  # 24.2
